...
function inputEnter() {
    ...
    // -> keypress (눌러진 상태) -> (손을 떼면) keyup

    // console.log(window.event.key); //  현재 눌러진 키 확인
    // keydown : 모든 키가 눌렸을 때(space, enter, 방향키 등 모두)
    // keypress : 영어/숫자/특수문자/띄어쓰기가 눌러지고 있을 때(input 태그에 입력 가능한 경우()
...
function readValue(){ // 입력된 값을 읽어들여 채팅창에 출력
    ...
    if(input.value.trim().length>0) {
        ...
        // 스크롤을 제일 밑으로 내리기
        // 요소.scrollTop : 요소내부 현재 스크롤의 시작 위치
