...
// box 클래스 요소에 높이 150px, 아랫쪽 테구리 1px 실선 검정색
for(let item of box) {
...
for(let i=0; i<boxColor.length; i++){
    // change : 포커스를 잃고나서, 작성된 값이 이전 값과 다를 경우, enter 인식 o
    // blur : 포커스를 잃었을 때, 내용이 변하지 않아도 인식함, enter 인식 x
    boxColor[i].addEventListener("change", function(){
        ...
        // this : change 이벤트가 발생한 요소이기 때문에 this로 해도 가능
    })
};
...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    for(let item of box) { // 박스 배열에 담긴 요소를 하나씩 꺼내 item에 저장
...
document.getElementById("clearBtn").addEventListener("click", function(){
    for(let item of box) {item.style.backgroundColor = ""};
    for(let item of boxColor){item.value = "";}
